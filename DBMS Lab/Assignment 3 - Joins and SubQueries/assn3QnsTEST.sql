...
SELECT COUNT(DISTINCT RNO) AS RECEIPT_COUNT
FROM ITEM_LIST
WHERE ITEM IN (SELECT PID
FROM PRODUCTS OUTER
WHERE PRICE >(SELECT AVG(PRICE)
FROM PRODUCTS
WHERE FOOD=OUTER.FOOD));
...
SELECT DISTINCT CID, FNAME, LNAME, RNO
FROM CUSTOMERS NATURAL JOIN RECEIPTS 
WHERE RNO IN (SELECT RNO
FROM (SELECT DISTINCT RNO, FLAVOR
    FROM PRODUCTS JOIN ITEM_LIST ON(ITEM=PID)
    WHERE FOOD='Meringue'
    GROUP BY RNO,FLAVOR)
GROUP BY RNO
HAVING COUNT(*)>1);
